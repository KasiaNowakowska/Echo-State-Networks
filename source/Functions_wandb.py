...
def step(x_pre, u, sigma_in, rho, params):
    ...
    bias_in       = params["bias_in"]
    bias_out      = params["bias_out"]
    u_mean        = params["u_mean"]
    norm          = params["norm"]
    norm_std      = params["norm_std"]
    norm_std_pr   = params["norm_std_pr"]
    ...
    if normalisation == 'on':
        ...
        u_augmented = np.hstack(((u[:n_components]-u_mean)/norm_std_pr, u[n_components:], bias_in))
    # reservoir update
...
def open_loop(U, x0, sigma_in, rho, params):
    ...
    for i in np.arange(1,N+1):
        Xa[i] = step(Xa[i-1,:N_units], U[i-1], sigma_in, rho, params)

    return Xa

def closed_loop(N, x0, Wout, sigma_in, rho, params):
    ...
    Yh[0] = np.dot(xa, Wout)
    for i in np.arange(1,N+1):
        xa    = step(xa[:N_units], Yh[i-1], sigma_in, rho, params)
        Xa[i] = xa
        Yh[i] = np.dot(xa, Wout) #np.linalg.multi_dot([xa, Wout])

    return Yh, xa, Xa

def train_n(U_washout, U_train, Y_train, tikh, sigma_in, rho, params, k=0):
    """ Trains ESN.
        Args:
    ...
    xf = open_loop(U_washout, np.zeros(N_units), sigma_in, rho, params)[-1,:N_units]
    ...
    for ii in range(N_splits):
        ...
        t1  = time.time()
        Xa1 = open_loop(U_train[ii*N_len:(ii+1)*N_len], xf, sigma_in, rho, params)[1:]
    ...
    if N_splits > 1:
        Xa1 = open_loop(U_train[(ii+1)*N_len:], xf, sigma_in, rho, params)[1:]
        LHS += np.dot(Xa1.T, Xa1)
...
def train_save_n(U_washout, U_train, Y_train, tikh, sigma_in, rho, noise, params):
    """ Trains ESN.
        Args:
            U_washout: washout input time series
            U_train: training input time series
