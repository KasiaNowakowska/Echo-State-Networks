...
input_path='./ToyData/'
output_path='./ToyData/Wavelet_Transform/newcwt/'
...
def dis_wavelet_transform(signal, level, wavelet = 'db4', z_value=32):
    ...
    wave_coeffs = [np.zeros_like(coeffs[0])] + [c for c in coeffs[1:]]
...
#dis_wavelet_transform(signal, 4, wavelet='db4', z_value=32)
