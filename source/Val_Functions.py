...
def RVC_Noise_PH_median(x):
    ...
    lenn     = tikh.size
    Mean     = np.zeros(lenn)
    PH       = np.zeros((lenn, N_fo))  # shape: (num_tikh, num_folds)
    ...
    #select optimal tikh
    #a           = np.argmin(Mean)
    median_PH    = np.median(PH, axis=1)
    a            = np.argmin(median_PH)
    tikh_opt[k] = tikh[a]
    k          +=1
    ...
    if print_flag:
        print(k, ': Spectral radius, Input Scaling, Tikhonov, MSE:',
              rho, sigma_in, tikh_opt[k-1],  median_PH[a])
